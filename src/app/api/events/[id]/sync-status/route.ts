...
async function syncStatuses(eventId: string, supabase: Awaited<ReturnType<typeof createClient>>) {
  ...
  if (!event.google_event_id) {
    return { success: true, updated: 0, updates: [], message: 'No Google event linked' };
  }
 ...
 * Synchronizace statusů attendees z Google Calendar (pro automatické polling)
 */
export async function GET(
  ...
  try {
    ...
    const result = await syncStatuses(eventId, supabase);
    ...
    console.error('Status sync error (GET):', error);
